...
from src.ae_utils import get_critical_pc_non_critical_pc
...
print('Run defense critical flags:', flags)
...
for i in range(len(pc_classes)):
    ...
    adversarial_critical_points = np.zeros([num_dist_weight, num_pc, bottleneck_size, 3], dtype=adversarial_pc_input.dtype)
    adversarial_critical_idx = np.zeros([num_dist_weight, num_pc, bottleneck_size], dtype=np.int16)
    adversarial_critical_num = np.zeros([num_dist_weight, num_pc], dtype=np.int16)
    ...
    for j in range(num_dist_weight):
        ...
        pre_symmetry_data = ae.get_pre_symmetry_data(pc_input)  # data before symmetry operation

        critical_points, critical_idx, critical_num, pc_critical, pc_defended = \
            get_critical_pc_non_critical_pc(pc_input, pre_symmetry_data)

        adversarial_critical_points[j], adversarial_critical_idx[j], adversarial_critical_num[j] = \
            critical_points, critical_idx, critical_num

        # sanity check
        if flags.do_sanity_checks:
            critical_recon = ae.get_reconstructions(pc_critical)
            assert np.array_equal(adv_recon, critical_recon), \
                'Reconstructions of adversarial point clouds and of adversarial critical points should be equal!'
    ...
    # save results
    np.save(osp.join(save_dir, 'adversarial_critical_points'), adversarial_critical_points)
    np.save(osp.join(save_dir, 'adversarial_critical_idx'), adversarial_critical_idx)
    np.save(osp.join(save_dir, 'adversarial_critical_num'), adversarial_critical_num)
    ...
    ##########################################################################
    source_pre_symmetry_data = ae.get_pre_symmetry_data(source_pc)  # data before symmetry operation

    s_critical_points, s_critical_idx, s_critical_num, s_pc_critical, s_pc_defended = \
        get_critical_pc_non_critical_pc(source_pc, source_pre_symmetry_data)

    # sanity check
    if flags.do_sanity_checks:
        s_critical_recon = ae.get_reconstructions(s_pc_critical)

        diff_critical_recon_max = np.abs(source_recon_ref - s_critical_recon).max()
        assert diff_critical_recon_max < 1e-06, \
            'Reconstructions of source point clouds and of source critical points should also be the same! (up to precision errors)'
    ...
    # save results
    np.save(osp.join(save_dir_orig, 'original_source_critical_points'), s_critical_points)
    np.save(osp.join(save_dir_orig, 'original_critical_idx'), s_critical_idx)
    np.save(osp.join(save_dir_orig, 'original_critical_num'), s_critical_num)
